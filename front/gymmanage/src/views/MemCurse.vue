    ...
    <div class="app-container">
      ...
      <div class="box">
        ...
        <div class="add-form">
          ...
          <el-dialog title="公开课购买" v-model="dialogFormVisible">
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      //发送ajax请求
      axios.post('http://localhost:8080/purcurse/save', this.formData).then((res) => {
        console.log(res.data);
        //如果操作成功，关闭弹层，显示数据
    ...
    handleDelete(row) {
      ...
      this.$confirm("This action will delete it forever,Confirm?", "warning", {
        ...
        //2.做删除业务
        axios.delete('http://localhost:8080/purcurse/' + row.orderid).then((res) => {
          res.data
        }).finally(() => {
          this.getOrderAll();
        });
